...
class EditCategoryViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, NSFetchedResultsControllerDelegate, UIGestureRecognizerDelegate {
    fileprivate let reuseIdentifier = "EditCategoryCell"
    ...
    var editCategory: UIBarButtonItem!
    ...
    func tapTimerFired(aTimer:Timer) {
        if tapTimer != nil {
            tapCount = 0
            tappedRow = -1
        }
...
extension CategoriesViewController {
    ...
    //            tapTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(tapTimerFired(aTimer:)), userInfo: nil, repeats: false)
    //
    //            self.performSegue(withIdentifier: "CategoryDetails", sender: cell)
    //        }
